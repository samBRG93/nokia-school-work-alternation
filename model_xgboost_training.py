...
from plots import plot_loss, plot_scatter, plot_temperature
...
def register_ml_flow_model(model, X_train, mse, mae, params):
    ...
    mlflow.set_experiment("XGBoost Regression Model London Weather")
    ...
    with mlflow.start_run():
        ...
        for i in enumerate(results['validation_0']['rmse']):
            mlflow.log_metric(key="training_loss", value=i[1], step=i[0])

        for i in enumerate(results['validation_1']['rmse']):
            mlflow.log_metric(key="validation_loss", value=i[1], step=i[0])

        mlflow.log_table(data=get_feature_importance(X_train, model), artifact_file="feature_importance.json")

        mlflow.set_tag("Training Info", "Xgboost Regression Model for London weather")
